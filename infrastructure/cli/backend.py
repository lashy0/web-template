...
def command_environment(environment: Environment) -> dict[str, str]:
    ...
    if environment is Environment.PROD:
        values["BACKEND_TAG"] = f"{version}-{read_git_sha(find_tool('git'))}"
    return values
...
def up(
    ...
    if environment is Environment.DEV:
        message = "Starting the backend development environment"
        arguments = ("up", "--build", "--watch")
    else:
        message = f"Deploying backend release {values['BACKEND_TAG']}"
    ...
    if environment is Environment.PROD:
        typer.secho("Backend is running.", fg=typer.colors.GREEN, bold=True, err=True)
...
def down(
    ...
    command = PROJECT.prepare(docker, environment, process_environment=values)
    typer.secho(
        f"Stopping the backend ({environment.value})",
        fg=typer.colors.CYAN,
        bold=True,
        err=True,
    )
    run_command([*command, "down"], process_environment=values)
    typer.secho("Backend is stopped.", fg=typer.colors.GREEN, bold=True, err=True)
