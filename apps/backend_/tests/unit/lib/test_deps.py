...
async def mock_provider(session: AsyncSession) -> AsyncGenerator[MockService]:
...
async def test_provide_services_standalone_mode() -> None:
    ...
    async def mock_get_session() -> AsyncGenerator[MagicMock]:
        yield mock_session
...
async def test_provide_services_multiple_providers() -> None:
    ...
    async def provider1(session: AsyncSession) -> AsyncGenerator[Service1]:
        yield Service1(session=session)

    async def provider2(session: AsyncSession) -> AsyncGenerator[Service2]:
