...
def test_database_url_is_built_from_backend_connection_fields() -> None:
    settings = Settings.model_validate(
        {
            ...
            "BACKEND_POSTGRES_RUNTIME_PASSWORD": SecretStr("secret"),
    ...
    assert (
        ...
        == "postgresql+psycopg://otk_app_runtime:secret@postgres.internal:5433/app_db"
    )
...
def test_legacy_postgres_connection_fields_are_supported() -> None:
    ...
    assert (
        str(settings.database_url)
        == "postgresql+psycopg://otk_app_runtime@postgres.internal:5432/app_db"
    )


@pytest.mark.unit
def test_runtime_password_from_backend_environment_is_supported() -> None:
    settings = Settings.model_validate(
        {
            "BACKEND_POSTGRES_RUNTIME_PASSWORD": SecretStr("runtime-secret"),
        }
    )
    ...
    settings = Settings.model_validate(
        {
            ...
            "POSTGRES_DB": "app_db",
            "BACKEND_POSTGRES_RUNTIME_PASSWORD": SecretStr("runtime-secret"),
            "BACKEND_POSTGRES_MIGRATOR_PASSWORD": SecretStr("migrator-secret"),
        }
    )

    assert (
        str(settings.migration_database_url)
        == "postgresql+psycopg://otk_app_migrator:migrator-secret@postgres.internal:5433/app_db"
    )
...
def test_migration_database_url_requires_the_migrator_password() -> None:
    settings = Settings.model_validate(
        {
            "BACKEND_POSTGRES_MIGRATOR_PASSWORD": None,
        }
    )

    with pytest.raises(ValueError, match="BACKEND_POSTGRES_MIGRATOR_PASSWORD is required"):
