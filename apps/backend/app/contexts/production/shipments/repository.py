...
class ShipmentRepository:
    """Shipment persistence and read projections; transaction ownership remains with callers."""

    def __init__(self, session: AsyncSession) -> None:
        self._session = session

    async def create(
        self, *, batch_id: UUID, comment: str | None, created_by_user_id: UUID | None
    ) -> BatchShipment:
        shipment = BatchShipment(
            batch_id=batch_id, comment=comment, created_by_user_id=created_by_user_id
        )
        self._session.add(shipment)
        await self._session.flush()
        return await self.refresh(shipment)

    async def get(self, shipment_id: UUID, *, for_update: bool = False) -> BatchShipment | None:
        return await self._session.get(
            BatchShipment,
            shipment_id,
            with_for_update=for_update,
            populate_existing=for_update,
            options=(selectinload(BatchShipment.created_by_user),),
        )

    async def get_by_id(self, shipment_id: UUID) -> BatchShipment | None:
        return await self.get(shipment_id)

    async def refresh(self, shipment: BatchShipment) -> BatchShipment:
        await self._session.refresh(shipment)
        await self._session.refresh(shipment, attribute_names=["created_by_user"])
        return shipment

    async def update(
        self, shipment: BatchShipment, *, updates: Mapping[str, object]
    ) -> BatchShipment:
    ...
    async def update_details(
        ...
        return await self.update(shipment, updates=updates)

    async def complete(self, shipment: BatchShipment, *, completed_at: datetime) -> BatchShipment:
        shipment.completed_at = completed_at
        await self._session.flush()
        return await self.refresh(shipment)

    async def void(
        self, shipment: BatchShipment, *, voided_at: datetime, reason: str
    ) -> BatchShipment:
    ...
    async def add_item(self, *, shipment_id: UUID, kg_dev_eui: str) -> BatchShipmentItem:
        item = BatchShipmentItem(shipment_id=shipment_id, kg_dev_eui=kg_dev_eui)
    ...
    async def get_item(self, *, shipment_id: UUID, kg_dev_eui: str) -> BatchShipmentItem | None:
        return await self._session.get(BatchShipmentItem, (shipment_id, kg_dev_eui))
    ...
    async def list_items(self, shipment_id: UUID) -> list[BatchShipmentItem]:
        result = await self._session.execute(
            ...
            .order_by(BatchShipmentItem.created_at.asc(), BatchShipmentItem.kg_dev_eui.asc())
        )
        return list(result.scalars())

    async def item_counts_by_batch(self, batch_id: UUID) -> dict[UUID, int]:
        result = await self._session.execute(
            select(BatchShipmentItem.shipment_id, func.count())
            .join(BatchShipment, BatchShipment.id == BatchShipmentItem.shipment_id)
    ...
    async def count_items_by_batch(self, batch_id: UUID) -> dict[UUID, int]:
        return await self.item_counts_by_batch(batch_id)

    async def item_count(self, shipment_id: UUID) -> int:
    ...
    async def count_items(self, shipment_id: UUID) -> int:
        return await self.item_count(shipment_id)

    async def find_non_voided_by_kg(self, kg_dev_eui: str) -> BatchShipment | None:
        result = await self._session.execute(
            select(BatchShipment)
            .join(BatchShipmentItem, BatchShipmentItem.shipment_id == BatchShipment.id)
            .where(BatchShipmentItem.kg_dev_eui == kg_dev_eui, BatchShipment.voided_at.is_(None))
            .limit(1)
        )
    ...
    async def list_by_batch(
        ...
        filters: list[ColumnElement[bool]] = [BatchShipment.batch_id == batch_id]
        ...
        result = await self._session.execute(
            ...
            .options(selectinload(BatchShipment.created_by_user))
            .where(*filters)
            .order_by(BatchShipment.created_at.desc(), BatchShipment.id.asc())
        )
        return list(result.scalars())

    async def shipped_total(self, batch_id: UUID) -> int:
        """Reporting total: items of completed, non-voided shipments only."""
