...
class VerificationSession(Base):
    ...
    __table_args__ = (
        CheckConstraint("slot_no > 0", name="verification_session_slot_no_positive"),
        CheckConstraint("total_steps > 0", name="verification_session_total_steps_positive"),
        Index("ix_verification_sessions_kg_started_at", "kg_dev_eui", "started_at"),
        Index("ix_verification_sessions_pak_started_at", "pak_id", "started_at"),
        Index("ix_verification_sessions_status_last_activity_at", "status", "last_activity_at"),
        Index(
...
class VerificationStep(Base):
    ...
    pak_test_id: Mapped[UUID] = mapped_column(
        Uuid(as_uuid=True), ForeignKey("pak_tests.id", ondelete="RESTRICT"), nullable=False
    )
    ...
    status: Mapped[VerificationStepStatus] = mapped_column(
        VERIFICATION_STEP_STATUS_DB_TYPE, nullable=False, default=VerificationStepStatus.RUNNING
    )
    ...
    __table_args__ = (
        CheckConstraint("step_no > 0", name="verification_step_no_positive"),
        CheckConstraint(
            "measurement_min_value IS NULL OR measurement_max_value IS NULL OR measurement_min_value <= measurement_max_value",
            name="verification_step_measurement_range_valid",
        ),
        UniqueConstraint("session_id", "step_no", name="uq_verification_steps_session_step_no"),
        Index("ix_verification_steps_test_name", "test_name"),
        Index("ix_verification_steps_status", "status"),
        Index("ix_verification_steps_pak_test_id", "pak_test_id"),
        Index("ix_verification_steps_defect_group_id", "defect_group_id"),
    )
