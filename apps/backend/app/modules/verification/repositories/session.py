...
from ..models import VerificationSession, VerificationSessionStatus
...
class VerificationSessionRepository:
    ...
    async def lock_running_candidates(
        ...
        *,
        ...
        """Lock affected sessions before any KG, in a deterministic row order."""
    ...
    async def exists_by_pak_id(self, pak_id: UUID) -> bool:
        return bool(
            await self._session.scalar(
                select(exists().where(VerificationSession.pak_id == pak_id))
            )
        )
    ...
    async def exists_by_kg_dev_eui(self, kg_dev_eui: str) -> bool:
        return bool(
            await self._session.scalar(
                select(exists().where(VerificationSession.kg_dev_eui == kg_dev_eui))
            )
        )

    async def exists_by_batch_id(self, batch_id: UUID) -> bool:
        return bool(
            await self._session.scalar(
                select(
                    exists().where(
    ...
    async def lock_session_open(
        ...
        *,
        ...
        keys = (
            ...
            f"verification:pak-slot:{pak_id}:{slot_no}",
        )
