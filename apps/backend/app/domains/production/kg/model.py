...
class KgState(StrEnum):
    """Persistent KG lifecycle state; shipment membership is not a KG state."""
...
class KgUnit(Base):
    ...
    state: Mapped[KgState] = mapped_column(
        KG_STATE_DB_TYPE, nullable=False, default=KgState.REGISTERED
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, server_default=func.now()
    )
...
class LoRaWanCredentials(Base):
    ...
    kg_dev_eui: Mapped[str] = mapped_column(
        String(16), ForeignKey("kg_units.dev_eui", ondelete="CASCADE"), primary_key=True
    )
    ...
    def __repr__(self) -> str:
        return f"LoRaWanCredentials(kg_dev_eui={self.kg_dev_eui!r}, schema_version={self.schema_version!r})"
