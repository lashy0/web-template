"""AES-GCM credential encryption, decryption, and schema-v1 serialization."""
...
def resolve_encryption_key(secret: SecretStr | None) -> bytes:
    ...
    return decode_encryption_key(secret.get_secret_value())
...
def _aad(context: CredentialsEncryptionContext, schema_version: int) -> bytes:
    return json.dumps(
        {
